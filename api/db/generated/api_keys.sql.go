...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, user_id, name, key_hash, key_prefix, scopes, expires_at, last_used_at, is_active, created_at, updated_at, logto_m2m_app_id
`
...
const getAPIKeyByLogtoM2MAppID = `-- name: GetAPIKeyByLogtoM2MAppID :one

SELECT id, user_id, name, key_hash, key_prefix, scopes, expires_at, last_used_at, is_active, created_at, updated_at, logto_m2m_app_id FROM api_keys WHERE logto_m2m_app_id = $1
`
...
SELECT id, user_id, name, key_hash, key_prefix, scopes, expires_at, last_used_at, is_active, created_at, updated_at, logto_m2m_app_id FROM api_keys WHERE id = $1
`
...
const setAPIKeyLogtoM2MAppID = `-- name: SetAPIKeyLogtoM2MAppID :one
UPDATE api_keys SET
    logto_m2m_app_id = $2,
    updated_at = now()
WHERE id = $1
...
type SetAPIKeyLogtoM2MAppIDParams struct {
	ID            int64   `json:"id"`
	LogtoM2mAppID *string `json:"logto_m2m_app_id"`
}

func (q *Queries) SetAPIKeyLogtoM2MAppID(ctx context.Context, arg SetAPIKeyLogtoM2MAppIDParams) (ApiKey, error) {
	row := q.db.QueryRow(ctx, setAPIKeyLogtoM2MAppID, arg.ID, arg.LogtoM2mAppID)
	var i ApiKey
	err := row.Scan(
		&i.ID,
		&i.UserID,
